          ...
          arrows.color=NULL,...) 
{
  ...
  if (inherits(pcobj, "prcomp")) {
  ...
  else if(setequal(names(pcobj), ## is a CCA object
    ...
    v <- rbind(pcobj$scores$corr.X.xscores,pcobj$scores$corr.Y.xscores)
    report="correlation"
  ...
  r <- sqrt(qchisq(circle.prob, df = 2)) * prod(colMeans(df.u^2))^(1/4)*2
  v.scale <- rowSums(v^2)
